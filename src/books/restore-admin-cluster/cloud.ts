...
class GenericAdminClusterCloudRestorer {
  ...
  async Run([adminLandscape, adminCluster]: LandscapeCluster): Promise<void> {
    ...
    await this.task.Run(["Get Backup Name", async () => {
    ...
    await this.task.Run(["Restore Backup", async () => {
      await $`nix develop -c velero --kubecontext ${{raw: context}} restore create --from-backup ${{raw: backupName}}`.cwd(F_Dir);
    }]);
